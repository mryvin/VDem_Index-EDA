...
## Sections 0 : Viewing the Democracy Indices by Continent

In this section, we will look at the democracy indices in a more general sense, by generalizing them by continent. Each of these will be subsetted to only include data points from the year 2022, so that we can see the modern democracy indices rather than the historical ones. 
...
## Section 1 : Analysis of Major countries vs High Fluctuating countries for all the 5 indices
...
## Section 2: Slope Coefficients of Electoral Democracy Index
...
## Section 3: Countries with the Highest Democracy Indices
...
**If you would like to see the interactivity, please uncomment "#runtime: shiny" at the top of this rmd**
```{r}
...
## Section 4: Evolution of Country Names over Time
...
## Section 5: Leaders and Electoral Democracy Index
...
ggplot(usa_data, aes(x = year, y = v2x_polyarchy, color = v2exnamhos, 
  ...
  theme_fivethirtyeight(base_size = 9.8)
```
...
ggplot(rus_data, aes(x = year, y = v2x_polyarchy, color = v2exnamhos, 
  ...
  theme_bw() +
  scale_color_manual(values = setNames(palette, v2ex_levels)) +
  theme_fivethirtyeight(base_size = 7.5)
```
